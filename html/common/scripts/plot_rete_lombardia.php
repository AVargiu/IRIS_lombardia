...
if($id_parametro=='PLUV') {
	...
	//Con questa query l'intervallo temporale e' deciso dai dati:
	...
	$query_cumulata = "SELECT data, sum(valore_originale) OVER (ORDER BY data ASC) FROM (SELECT to_char(to_timestamp(data||ora, 'YYYY-MM-DDHH24:MI:SS')::timestamp - '1 minute'::interval,'YYYY-MM-DD HH24:00:00')::timestamp + '1 hour'::interval AS data, sum(valore_originale) AS valore_originale FROM $realtimetable_from a WHERE a.id_stazione = $cod_staz AND id_parametro = '$id_parametro' GROUP BY to_char(to_timestamp(data||ora, 'YYYY-MM-DDHH24:MI:SS')::timestamp - '1 minute'::interval,'YYYY-MM-DD HH24:00:00')::timestamp + '1 hour'::interval) AS foo;";
}
...
$realtimetable_from = 'expo2015.meteo_real_time_lombardia';
	$query = "SELECT realtime.cardinal2degree(dirv.direction), round((count(velv.*) * 100 / max(dirv.total))) AS big_freq0, round((count(velv1.*) * 100 / max(dirv.total))) AS big_freq1, round((count(velv2.*) * 100 / max(dirv.total))) AS big_freq2, round((count(velv3.*) * 100 / max(dirv.total))) AS big_freq3, max(dirv.total) AS big_total, max(data_agg) as data_agg FROM (SELECT id_stazione, dataora, data_agg, count(*) OVER () AS total, realtime.degree2cardinal(valore_originale) AS direction FROM $realtimetable_from WHERE id_parametro IN ('DIRV') AND valore_originale IS NOT NULL AND id_stazione=$cod_staz AND to_date(dataora::text, 'YYYYMMDDHH24MI'::text) = current_date) AS dirv LEFT JOIN (SELECT valore_originale AS speed0, id_stazione, dataora FROM $realtimetable_from WHERE id_parametro IN ('VELV') AND id_stazione = $cod_staz AND valore_originale IS NOT NULL AND valore_originale < 0.5 AND to_date(dataora::text, 'YYYYMMDDHH24MI'::text) = current_date) AS velv ON (velv.id_stazione= dirv.id_stazione AND velv.dataora = dirv.dataora) LEFT JOIN (SELECT valore_originale AS speed1, id_stazione, dataora FROM $realtimetable_from WHERE id_parametro IN ('VELV') AND id_stazione = $cod_staz AND valore_originale IS NOT NULL AND valore_originale BETWEEN 0.5 AND 5 AND to_date(dataora::text, 'YYYYMMDDHH24MI'::text) = current_date) AS velv1 ON (velv1.id_stazione= dirv.id_stazione AND velv1.dataora = dirv.dataora) LEFT JOIN (SELECT valore_originale AS speed2, id_stazione, dataora FROM $realtimetable_from WHERE id_parametro IN ('VELV') AND id_stazione = $cod_staz AND valore_originale IS NOT NULL AND valore_originale BETWEEN 5 AND 10 AND to_date(dataora::text, 'YYYYMMDDHH24MI'::text) = current_date) AS velv2 ON (velv2.id_stazione= dirv.id_stazione AND velv2.dataora = dirv.dataora) LEFT JOIN (SELECT valore_originale AS speed3, id_stazione, dataora FROM $realtimetable_from WHERE id_parametro IN ('VELV') AND id_stazione = $cod_staz AND valore_originale IS NOT NULL AND valore_originale > 10 AND to_date(dataora::text, 'YYYYMMDDHH24MI'::text) = current_date) AS velv3 ON (velv3.id_stazione= dirv.id_stazione AND velv3.dataora = dirv.dataora) WHERE dirv.id_stazione = $cod_staz GROUP BY dirv.id_stazione, dirv.direction ORDER BY dirv.id_stazione, realtime.cardinal2degree(dirv.direction);";
}
else {
