...
FROM generate_series (now()-'24 hour'::interval , now(), '1 hour'::interval) dd
LEFT OUTER JOIN (
SELECT data+ora::time AS dataora, sum(valore_originale) OVER (ORDER BY data+ora::time) FROM $realtimetable_from WHERE codice_istat_comune = '$codice_istat' AND progr_punto_com = $progr_punto_com AND id_parametro ='PLUV' AND data+ora::time >= now()-'24 hour'::interval
...
EOT;
    ...
    $query = <<<EOT
                SELECT (dd - '00:00:01'::interval)::date as data, dati_staz01.denominazione, dati_staz01.valore AS spi01 FROM generate_series ('1958-02-01'::date , current_date, '1 month'::interval) dd
                LEFT OUTER JOIN (SELECT data::date AS data, valore, denominazione FROM realtime.bacini_siccita_storico f, dati_di_base.bacini_siccita h
                WHERE f.codice = '$codice_istat' AND h.codice = '$codice_istat' AND id_parametro like '$parametri[0]') AS dati_staz01 ON ((dd - '00:00:01'::interval)::date=dati_staz01.data)
                ORDER BY data ASC;
EOT;
	...
	$query = <<<EOT
        ...
        , mc_model.id_previ, mc_model.dataora_previ, m1_model.id_previ, m1_model.dataora_previ, so_model.id_previ, so_model.dataora_previ, he_model.id_previ, he_model.dataora_previ
...
EOT;
        ...
        , m1.id_previ, m1.dataora_previ, m2.id_previ, m2.dataora_previ, m3.id_previ, m3.dataora_previ, m4.id_previ, m4.dataora_previ
        FROM generate_series (now()-'24 hour'::interval , now()+'72 hour'::interval, '1 hour'::interval) dd
        LEFT OUTER JOIN (
...
EOT;
        ...
        $query = <<<EOT
	SELECT date_trunc('hour', dd)::timesta$parametri[0] without time zone AS data, max_modello.valore AS max_modello, min_modello.valore AS min_modello, dati_oss.valore AS oss, avg_modello.valore AS avg
        , max_modello.dataora_forecast AS max_modello_tof, min_modello.dataora_forecast AS min_modello_tof
        , max_modello.id_previ AS max_id_previ, max_modello.dataora_previ AS max_dataora_previ, min_modello.id_previ AS min_id_previ, min_modello.dataora_previ AS min_dataora_previ, onlymax.valore AS valorimax, onlymin.valore AS valorimin
        FROM generate_series (now()-'24 hour'::interval , now()+'72 hour'::interval, '1 hour'::interval) dd
        LEFT OUTER JOIN (
        SELECT to_timesta$parametri[0](data||' '||ora, 'YYYY-MM-DD HH24:MI')::timesta$parametri[0] without time zone AS dataora_valore, valore_validato AS valore FROM realtime.meteo_real_time WHERE codice_istat_comune||progr_punto_com='$codice_istat$progr_punto_com' AND id_parametro = 'PORTATA'
        ) AS dati_oss ON dati_oss.dataora_valore=date_trunc('hour', dd)
        LEFT OUTER JOIN (
	...
	*/
	...
	legend: {
		...
		,plotLines: [
			    ...
			    else if (id_parametro == 'IDRO_BIS' || id_parametro == 'PORTATA_BIS' || id_parametro == 'bilancioIDRO') {
                                return Highcharts.dateFormat('%d/%m/%y', this.value);
                            }
			    //else if (id_parametro == 'SPI03') {
