...
if (!$conn_local) { // Check if valid connection
    ...
    $query_indice = "SELECT webgis_idx, zoom_center, lon_center, lat_center, query_map_idx, tools_variable, baselayers, attivo, collapsed_grid, store_grid, columns_grid, title_grid FROM config.v_webgis_general_config WHERE webgis_name = '$webgis_type';";
	...
	if ($attivo == 0 && $devel == 0) {
	  ...
	  pg_close($conn_local);
	  die("<script>location.href = '" . $root_dir_html . "/nonattivo.html?error=". $error_message ."'</script>");
	}
