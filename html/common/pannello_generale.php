...
if (!$conn) { // Check if valid connection
    ...
    if (!$result_webgis) {
		...
		foreach ($fetch_all_webgis as $fetch_webgis) {
			...
			$build_table .= "<tr><td><a title='" . $fetch_webgis['webgis_description'] .  "'href='" . $root_dir_html . $root_dir_script . '?TYPE=' . $fetch_webgis['webgis_name'] . "' target='_blank' class='classname'>" . $fetch_webgis['webgis_name'] . "</a></br></tr></td>";
		    }
		    //Al momento non visualizzo i webgis disattivati, ma posso sempre elencarli a fianco...
		    else $build_table_deactivate .= "<tr><td><a title='" . $fetch_webgis['webgis_description'] .  "'href='" . $root_dir_html . $root_dir_script . '?TYPE=' . $fetch_webgis['webgis_name'] . "' target='_blank' class='classname'>" . $fetch_webgis['webgis_name'] . "</a></br></tr></td>";
		}
	}
}
pg_close($conn);
...
<div style="text-align:center;">
		...
		?>
               ...
               <!--  <a href="" onClick="alert('Funzione non ancora attiva')" class="classname" style="font-size:1em; width:25%;padding:2px 3px;">Gestione layers</a></br> -->
             </td></tr>
