...
$nomelogo = $root_dir_html . "/common/icons/LOGO_ARPA_low.jpg";
$urllogo = "http://www.arpalombardia.it/meteo/";
...
case "guest":
	$local_path = "/guest/";
	$nomelogo = $root_dir_html . "/common/icons/logo_expo2015_transp.png";
        $urllogo = "http://www.expo2015.org/it";
        $titlelogo = "ARPA Lombardia";
        break;
case "expo2015":
        $local_path = "/expo2015/";
        $nomelogo = $root_dir_html . "/common/icons/logo_expo2015_transp.png";
...
case "arpalombardia":
        $local_path = "/users/";
        $nomelogo = $root_dir_html . "/common/icons/logo_arpapl_transp.png";
        break;
case "thefloatingpiers":
        $local_path = "/thefloatingpiers/";
        $nomelogo = $root_dir_html . "/common/icons/logo_arpapl_transp.png";
        break;
case "pclombardia":
        $local_path = "/pclombardia/";
        break;
default:
        $local_path = "/webgis/meteo/";
}
...
if (!$conn) { // Check if valid connection
	$error_message = "Error Connecting to database";
        die("<script>location.href = 'http://www.arpa.piemonte.gov.it/radar/error.html?error=". $error_message ."'</script>");
        //echo "Error Connecting to database <br>";
        //exit;
}
else {
    //Faccio una prima query per recuperare l'indice del webgis:
    if ($news_message) $query_news = "SELECT messaggio, icona_iniziale, gid FROM config.news_message WHERE data_scadenza IS NULL AND gid NOT IN ($news_message) ORDER BY gid;";
    else $query_news = "SELECT messaggio, icona_iniziale, gid FROM config.news_message WHERE data_scadenza IS NULL ORDER BY gid;";
