...
try:
  ...
  url=BASE_URL+"?locations="+str(y2)+","+str(x2)
...
select_tag = '<select id="srid" onChange="convert_srid(this.value)"> <option value="proj4326">WGS84 Lat/Lon</option> <option value="proj4326dd">WGS84 Lat/Lon DMS</option> <option value="proj32632">WGS84 UTM32N</option> <option value="proj4230">ED50 Lat/Lon </option><option value="proj4230dd">ED50 Lat/Lon DMS</option> <option value="proj23032">ED50 UTM32N</option> </select>'
...
query_com = "select localita, provincia, regione, istat_2014 from dati_di_base.limiti_amministrativi where st_intersects(ST_Transform(ST_GeomFromText('POINT(%s %s)', %s), 32632), the_geom);" % (x, y, srid)
...
#Elaboro la precipitazione istantanea:
  ...
  for row in table1:
	...
	"""
	  ...
	  if ("xband_" in row[0] or "_composite_" in row[0] or "_ist_bis" in row[0]):
	    continue
        elif webgis == 'arpalombardia':
          stop_raster = ["_composite_", "p_ist_"+subsrid]
          if ("_composite_" in row[0] or "p_ist_"+subsrid in row[0]):
            continue
        elif webgis == 'expo2015' or webgis == 'expo2015_pub':
